...
const PaymentManagement = () => {
  ...
  useEffect(() => {
    loadPaymentData();
  }, [searchTerm, statusFilter, paymentMethodFilter, currentPage, sortBy]);
  ...
  const loadPaymentData = async () => {
    try {
      ...
      Object.keys(transactionParams).forEach(key => 
        transactionParams[key] === undefined && delete transactionParams[key]
      );

      // CORRECTED: Use actual endpoints from your Django URLs
      const [paymentsRes, overviewRes, analyticsRes, refundStatsRes] = await Promise.all([
        // CORRECTED: GET /api/payments/admin/payments/ (AdminPaymentListView)
        axiosInstance.get(
          `/payments/admin/payments/`,
          { ...getAuthHeader(), params: transactionParams }
        ),
        // CORRECTED: GET /api/payments/admin/overview/ (PaymentOverviewView)
        axiosInstance.get(
          `/payments/admin/overview/`,
          getAuthHeader()
        ),
        // CORRECTED: GET /api/payments/admin/analytics/revenue/ (RevenueAnalyticsView)
        axiosInstance.get(
          `/payments/admin/analytics/revenue/`,
          getAuthHeader()
        ),
        // CORRECTED: GET /api/payments/admin/analytics/refunds/ (RefundStatisticsView)
        axiosInstance.get(
      ...
      // CORRECTED: Set transaction data based on actual API response
      if (paymentsRes.data.results) {
        setTransactions(paymentsRes.data.results);
      } else {
        // Fallback for non-paginated response
        setTransactions(paymentsRes.data || []);
      }
      
      setTotalCount(paymentsRes.data.count || paymentsRes.data.length || 0);
      setTotalPages(paymentsRes.data.total_pages || 1);

      // CORRECTED: Map overview data from actual API structure
      if (overviewRes.data) {
        setPaymentOverview({
      ...
      // CORRECTED: Set analytics data
      setRevenueAnalytics(analyticsRes.data || {});

      // CORRECTED: Set refund statistics
      ...
      setError(
        ...
        err.message || 
        'Failed to load payment data. Please check your backend connection.'
      );
    } finally {
      setLoading(false);
    }
